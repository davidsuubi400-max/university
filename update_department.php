...
if (isset($_POST['submit'])) {
    ...
    if (!empty($name) && !empty($faculty)) {
        $stmt = $connect->prepare("UPDATE department SET name = ?, faculty = ? WHERE id = ?");
        $stmt->bind_param("ssi", $name, $faculty, $id);
        if ($stmt->execute()) {
            // Redirect after successful update
            header("Location: view_department.php?update=success");
            exit;
        } else {
...
// Fetch department data if an ID is provided
if (isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $stmt = $connect->prepare("SELECT * FROM department WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $result = $stmt->get_result();
    if ($result->num_rows === 1) {
        $row = $result->fetch_assoc();
        $department_name = htmlspecialchars($row['name']);
        $faculty_name = htmlspecialchars($row['faculty']);
    } else {
...
<head>
    ...
    <title>Update Department | David Elementary University</title>
...
<div class="container-fluid px-0">
            ...
            <div class="col-lg-6 col-md-8">
                <div class="form-card">
                    <div class="card-header">
                        <i class="fas fa-edit"></i> Update Department Information
                    </div>
                    <div class="card-body p-4">
                        ...
                        <form method="post" action="">
                            ...
                            <div class="d-flex gap-3 justify-content-start">
                                ...
                                <a href="view_department.php" class="btn btn-back">
    ...
    <footer>
        <div class="container">
            ...
            <br><small>© <?php echo date("Y"); ?> | Department Management System</small>
        </div>
    </footer>
</div>
