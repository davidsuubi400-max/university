<?php
include 'config.php'; // Database connection (should set $connect)
...
if (isset($_POST['submit'])) {
    ...
    if (!empty($name)) {
        $stmt = $connect->prepare("UPDATE faculty SET name = ? WHERE id = ?");
        $stmt->bind_param("si", $name, $id);
        if ($stmt->execute()) {
            // Success – redirect with a success message (optional)
            header("Location: view_faculty.php?update=success");
            exit;
        } else {
...
// Get faculty data if an ID is provided
if (isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $stmt = $connect->prepare("SELECT * FROM faculty WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $result = $stmt->get_result();
    if ($result->num_rows === 1) {
        $row = $result->fetch_assoc();
        $faculty_name = htmlspecialchars($row['name']);
    } else {
...
<head>
    ...
    <title>Update Faculty | David Elementary University</title>
...
<div class="container-fluid px-0">
            ...
            <div class="col-lg-6 col-md-8">
                <div class="form-card">
                    <div class="card-header">
                        <i class="fas fa-edit"></i> Update Faculty Information
                    </div>
                    <div class="card-body p-4">
                        ...
                        <form method="post" action="">
                            <div class="form-group mb-4">
                                <label for="name" class="form-label fw-semibold">Faculty Name</label>
                            ...
                            <div class="d-flex gap-3 justify-content-start">
                                ...
                                <a href="view_faculty.php" class="btn btn-back">
    ...
    <footer>
        <div class="container">
            ...
            <br><small>© <?php echo date("Y"); ?> | Faculty Management System</small>
        </div>
    </footer>
</div>
