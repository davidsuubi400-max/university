...
if (isset($_POST['submit'])) {
    ...
    $yearofstudy = intval($_POST['yearofstudy']);
    $semester = intval($_POST['semester']);
    $department = trim($_POST['department']);
    $course = trim($_POST['course']);

    if (!empty($coursecode) && !empty($coursename) && !empty($department) && !empty($course)) {
        $stmt = $connect->prepare("UPDATE courseunity SET coursecode = ?, coursename = ?, yearofstudy = ?, semester = ?, department = ?, course = ? WHERE id = ?");
        $stmt->bind_param("ssiissi", $coursecode, $coursename, $yearofstudy, $semester, $department, $course, $id);
        if ($stmt->execute()) {
            header("Location: view_courseunit.php?update=success");
            exit;
        } else {
...
// Fetch course unit data if an ID is provided
if (isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $stmt = $connect->prepare("SELECT * FROM courseunity WHERE id = ?");
    ...
    if ($result->num_rows === 1) {
        ...
        $yearofstudy = htmlspecialchars($row['yearofstudy']);
...
<head>
    ...
    <title>Update Course Unit | David Elementary University</title>
            ...
            <div class="col-lg-7 col-md-9">
                <div class="form-card">
                    <div class="card-header">
                        <i class="fas fa-edit"></i> Update Course Unit
                    </div>
                        ...
                        <form method="post" action="">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" id="coursecode" name="coursecode" 
                                           value="<?php echo $coursecode; ?>" required>
                                </div>
                                <div class="col-md-6 mb-3">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" class="form-control" id="yearofstudy" name="yearofstudy" 
                                           value="<?php echo $yearofstudy; ?>" required>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="semester" class="form-label fw-semibold">Semester</label>
                                    <input type="number" class="form-control" id="semester" name="semester" 
                                           value="<?php echo $semester; ?>" required>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" id="course" name="course" 
                                           value="<?php echo $course; ?>" required>
                                </div>
                            </div>
                            <input type="hidden" name="id" value="<?php echo $id; ?>">
                            <div class="d-flex gap-3 justify-content-start mt-2">
                                <button type="submit" name="submit" class="btn btn-update text-white">
                                    <i class="fas fa-save me-1"></i> Update Course Unit
                                </button>
                                <a href="view_courseunit.php" class="btn btn-back">
